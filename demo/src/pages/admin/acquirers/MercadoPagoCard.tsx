...
import { QrCode, CreditCard, Barcode, Settings, Save, CheckCircle, XCircle, AlertCircle, Loader2, Wifi } from "lucide-react";
...
const methods = [
  ...
  { name: "Cartão", icon: <CreditCard className="h-5 w-5 text-primary" /> },
  { name: "Boleto", icon: <Barcode className="h-5 w-5 text-primary" /> },
];
...
function deriveStatus(active: boolean, hasCredentials: boolean): AcquirerStatus {
  if (active && hasCredentials) return "connected";
  if (hasCredentials) return "inactive";
  return "not_configured";
}

function StatusBadge({ status }: { status: BackendAcquirerStatus["status"] | AcquirerStatus }) {
  if (status === "connected") {
    return (
...
const MercadoPagoCard = ({ backendStatus, onOverviewChange }: MercadoPagoCardProps) => {
  ...
  const loadConfig = async ({ showToast }: { showToast: boolean }) => {
    ...
    const invoke = async (token: string) =>
      supabase.functions.invoke("admin-acquirer-config", {
        body: { action: "get", acquirer_name: "MERCADO PAGO" },
      });

    let { data: resp, error, response } = await invoke(accessToken);
    ...
    if (error) {
      console.error("MercadoPagoCard loadConfig error:", error);
      const respStatus = (response as any)?.status;
      if (showToast) {
      ...
      const pk = creds?.public_key || "";
      setAccessToken(at);
      setPublicKey(pk);
      setActive(!!payload?.active);
      setHasCredentials(!!(at.trim() || pk.trim()));
    }
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    ...
    const clearingConfig = !trimmedAccessToken && !trimmedPublicKey;

    if ((!trimmedAccessToken && trimmedPublicKey) || (trimmedAccessToken && !trimmedPublicKey)) {
      toast.error("Preencha os dois campos ou deixe ambos vazios para desativar.");
      return;
    }
    ...
    const invoke = async (token: string) =>
      supabase.functions.invoke("admin-acquirer-config", {
        body: { action: "upsert", acquirer_name: "MERCADO PAGO", credentials, active: !clearingConfig },
      });

    let { data: resp, error, response } = await invoke(authToken);
    const respStatus = (response as any)?.status;
    if (error && respStatus === 401) {
      const newToken = await getValidAccessToken();
    ...
    if (error) {
      console.error("MercadoPagoCard handleSave error:", error);
      const respStatus = (response as any)?.status;
      if (respStatus === 401) toast.error("Sessão expirada. Faça login novamente.");
      else if (respStatus === 403) toast.error("Seu usuário não é admin. Entre em /admin/login com um usuário admin.");
      else toast.error("Erro ao salvar configuração.");
    } else {
      toast.success(clearingConfig ? "Mercado Pago desativado com sucesso!" : "Mercado Pago configurado com sucesso!");
      const payload = unwrapAcquirerConfigPayload(resp);
      setActive(!!payload?.active);
      setHasCredentials(!clearingConfig);
      setAwaitingTest(!clearingConfig);
      setStatusOverride(clearingConfig ? "not_configured" : "pending_test");
  ...
  const handleTest = async () => {
    ...
    supabase.functions.setAuth(authToken);
    const { data: resp, error } = await supabase.functions.invoke("test-acquirer-connection", {
      body: { acquirer_name: "MERCADO PAGO" },
    });
    if (error) {
    ...
    <Card className="p-5 flex flex-col gap-4 h-full">
      ...
      {open && (
        <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-4">
          <div className="space-y-1.5">
            <Label htmlFor="mp-access-token" className="text-xs">Access Token</Label>
            <PasswordInput
              id="mp-access-token"
              placeholder="APP_USR-..."
              value={accessToken}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="mp-public-key" className="text-xs">Public Key</Label>
            <Input
              id="mp-public-key"
              type="text"
              placeholder="APP_USR-..."
